...
void producer_func(FractalMessageQueue * queue, EasyBMP::Image * img, float left, float top, float xsize, float ysize, float img_min_x, float img_min_y, float img_max_x, float img_max_y, float img_x_size, float img_y_size) {


    //std::shared_ptr<FractalMessage> msg = std::make_shared<FractalMessage>(img, left, top, xsize, ysize, img_min_x, img_min_y, img_max_x, img_max_y, img_x_size, img_y_size);
    queue->push(std::make_shared<FractalMessage>(img, left, top, xsize, ysize, img_min_x, img_min_y, img_max_x, img_max_y, img_x_size, img_y_size));
};
...
int main()
{
	...
	for (size_t j = 0; j < num_tiles_y; j++) {
		...
		producer_func(&queue, &img, fractal_tile_min_x, fractal_tile_min_y, fractal_tile_max_x, fractal_tile_max_y, img_tile_min_x, img_tile_min_y, img_tile_max_x, img_tile_max_y, img_size_x, img_size_y);
	...
	do {
		...
		cout << "Queue size is " + std::to_string(queue.size()) << endl;
	} while (queue.size() != 0);
